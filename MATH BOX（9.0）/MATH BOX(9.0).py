...
while True:
    ...
    if j1>0:
        ...
        a = input("请输入需要选择的功能的代码(0是生成质数表，1是翻倍运算，2是计算算数平方根，3是计算圆周率，4是求n次方，5是四则运算，6是生成斐波那契数列，7是计算黄金分割率,8是找零程序（贪心算法），9是统计，10是绘制函数图像，11是考试成绩数据整理，输入clock启动计时器，输入exit关闭程序，输入help查看使用指南,输入rr查看历史计算结果，输入cr清空历史计算结果")
        j1+=1
    #生成质数表
    if a=="0":
...
#帮助
    if a=="help":
        print("请输入需要选择的功能的代码(0是生成质数表，1是翻倍运算，2是计算算数平方根，3是计算圆周率，4是求n次方，5是四则运算，6是生成斐波那契数列，7是计算黄金分割率,8是找零程序（贪心算法），9是统计，10是绘制函数图像，11是考试成绩数据整理，输入clock启动计时器，输入exit关闭程序，输入help查看使用指南，输入rr查看历史计算结果，输入cr清空历史计算结果)")
    if a=="rr":
